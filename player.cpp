...
bool Player::updateSaleShopList(const Item* item)
{
    ...
    if (!isCurrency) {
        ...
        if (it == shopItemList.end()) {
            ...
            return std::any_of(items.begin(), items.end(), [this](const Item* containerItem) {
                std::cout << "Updating sale shop list for item ID: " << containerItem->getID() << "\n";
    ...
    if (client) {
        std::cout << "Sending updated sale shop list to client.\n";
...
bool Player::hasShopItemForSale(uint32_t itemId, uint8_t subType) const
{
    ...
    bool hasItemForSale = std::any_of(shopItemList.begin(), shopItemList.end(), [&](const ShopInfo& shopInfo) {
        bool meetsCriteria = shopInfo.itemId == itemId && shopInfo.buyPrice != 0 && (!itemType.isFluidContainer() || shopInfo.subType == subType);
        if (meetsCriteria) {
            std::cout << "Item ID: " << itemId << " is available for sale.\n";
        }
        return meetsCriteria;
    });

    if (!hasItemForSale) {
        std::cout << "Item ID: " << itemId << " is not available for sale.\n";
    }

    return hasItemForSale;
}
