...
void LuaScriptInterface::registerFunctions()
{
	...
	registerMethod("Item", "updateRarityFrame", LuaScriptInterface::luaUpdateRarityFrame);
...
int LuaScriptInterface::luaUpdateRarityFrame(lua_State* L) {
	// item:updateRarityFrame()
	Item* item = getUserdata<Item>(L, 1);
	if (!item) {
		lua_pushnil(L);
		return 1;
...
//LOSTEXPERIENCE
int LuaScriptInterface::luaPlayerGetLostExperience(lua_State* L)
{
	// player:getLostExperience()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->getLostExperience());
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaPlayerAddWings(lua_State* L) {
	...
	pushBoolean(L, player->addWings(wingId));
	return 1;
}
